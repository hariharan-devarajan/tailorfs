...
TEST_CASE("Read-Only", "[type=read-only][optimization=buffered_read]") {
    ...
    SECTION("storage.pfs") {
        ...
        for (int i = 0; i < args.iteration; ++i) {
            ...
            MPI_Status stat_orig;
            auto ret_orig = MPI_File_read_at_all(fh_orig, rank * args.request_size * args.iteration,
    ...
    SECTION("storage.unifyfs.buffer") {
        ...
        for (int iter = 0; iter < num_iter; ++iter) {
            ...
            for (int i = iter * num_req_to_buf; i < iter * num_req_to_buf + num_req_to_buf; ++i) {
                ...
                read_req[i].offset = i * args.request_size + (rank * args.request_size * args.iteration);
    ...
    MPI_Reduce(&close_a, &total_close, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&read_a, &total_read, 1, MPI_DOUBLE, MPI_SUM, 0,
               MPI_COMM_WORLD);
    ...
    if (rank == 0){
        printf("Timing\t\t,%ld\t,%ld\t,%f\t,%f\t,%f\t,%f\t,%f\t,%f,\t%s\n", args.iteration,
               args.request_size, total_init/comm_size,
               total_finalize/comm_size, total_open/comm_size,
               total_close/comm_size,
               total_read/comm_size,total_prefetch/comm_size,
               usecase);
    }
    MPI_Barrier(MPI_COMM_WORLD);
}
