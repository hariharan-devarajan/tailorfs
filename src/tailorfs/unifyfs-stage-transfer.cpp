 ...
 *
 */
static int read_unify_file_block(unifyfs_handle fshdl, unifyfs_gfid gfid,
                                 off_t file_offset, size_t bufsize,
                                 char* databuf, size_t* nread) {
  int ret = 0;
  size_t len = 0;
...
static int write_unify_file_block(unifyfs_handle fshdl, unifyfs_gfid gfid,
                                  off_t file_offset, size_t bufsize,
                                  char* databuf, size_t* nwrite) {
  int ret = 0;
  size_t len = 0;
...
static int read_file_block(int fd, off_t file_offset, size_t bufsize,
                           char* databuf, size_t* nread) {
  int err;
  int ret = 0;
  errno = 0;
  ...
  if (-1 == len) {
    err = errno;
    fprintf(stderr, "UNIFYFS-STAGE ERROR: pread() failed - %s", strerror(err));
    ret = err;
    len = 0;
  }
  *nread = len;
 ...
 */
static int md5_checksum(unifyfs_stage* ctx, bool is_unify_file,
                        const char* path, unsigned char* digest) {
  ...
  if (md5_rc != 1) {
    ...
    do {
      ...
      if (is_unify_file) {
        rc = read_unify_file_block(ctx->fshdl, gfid, file_offset, sizeof(data),
                                   (char*)data, &len);
      } else {
        rc = read_file_block(fd, file_offset, sizeof(data), (char*)data, &len);
      }
      if (rc != 0) {
        ret = EIO;
        break;
 ...
 */
static int verify_checksum(unifyfs_stage* ctx, bool src_in_unify,
                           bool dst_in_unify, const char* src,
                           const char* dst) {
  int ret = 0;
  int i = 0;
  char md5src[2 * MD5_DIGEST_LENGTH + 1] = {
      0,
  };
  char md5dst[2 * MD5_DIGEST_LENGTH + 1] = {
      0,
  };
  unsigned char src_digest[MD5_DIGEST_LENGTH + 1] = {
      0,
  };
  unsigned char dst_digest[MD5_DIGEST_LENGTH + 1] = {
      0,
  };
  ...
  if (ret) {
    fprintf(stderr,
            "UNIFYFS-STAGE ERROR: MD5 checksum failure "
            "for source file %s\n",
            src);
    return ret;
  }
  ...
  for (i = 0; i < MD5_DIGEST_LENGTH; i++) {
    if (src_digest[i] != dst_digest[i]) {
      fprintf(stderr,
              "UNIFYFS-STAGE ERROR: checksums do not match! "
              "(src=%s, dst=%s)\n",
...
static int distribute_source_file_data(unifyfs_stage* ctx,
  ...
  if (start_block_ndx < num_file_blocks) {
    ...
    char* block_data = (char*)malloc(transfer_blksz);
    if (NULL == block_data) {
      ret = ENOMEM;
      goto err_ret;
    }
    ...
    for (size_t i = 0; i < blocks_per_client; i++) {
      ...
      if (block_ndx < num_file_blocks) {
        ...
        rc = read_file_block(fd, block_offset, transfer_blksz, block_data,
                             &nread);
        if (rc) {
          ret = rc;
          break;
          ...
          size_t nwrite = 0;
          rc = write_unify_file_block(ctx->fshdl, gfid, block_offset, nread,
                                      block_data, &nwrite);
          if (rc) {
            ret = rc;
            break;
...
int unifyfs_stage_transfer(unifyfs_stage* ctx, int file_index,
  ...
  if (dst_in_unify) {
    ...
    rc = MPI_Bcast((void*)&src_file_size, 1, MPI_UNSIGNED_LONG, mgr_rank,
                   MPI_COMM_WORLD);
    if (rc != MPI_SUCCESS) {
  ...
  if (0 == ret) { /* transfer completed OK */
    if ((mgr_rank == ctx->rank) && (ctx->checksum)) {
      rc = verify_checksum(ctx, src_in_unify, dst_in_unify, src, dst);
      if (rc) {
        fprintf(stderr,
                "[rank=%d] UNIFYFS-STAGE ERROR: "
