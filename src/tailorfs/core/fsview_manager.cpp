...
TailorFSStatus tailorfs::FSViewManager::initialize() {
  ...
  if (mimir_intent_conf != nullptr) {
    ...
    if (mimir_intent_conf->_current_process_index == -1) {
      ...
      auto app_intent =
          mimir_intent_conf
              ->_app_repo[mimir_intent_conf->_current_process_index];
      ...
      if (app_intent._is_mpi) {
        ...
        TAILORFS_LOGINFO("app rank is %d", rank);
      }
      for (const auto& edge : app_intent._rank_file_dag.edges) {
        if (rank == edge.source) {
          ...
          auto workload_type =
              app_intent._file_workload.find(file_index)->second;
          auto access_pattern =
              app_intent._file_access_pattern.find(file_index)->second;
          if (file_intent._file_sharing ==
              mimir::FileSharing::FILE_SHARED_COLLECTIVE) {
            if (workload_type == mimir::WorkloadType::READ_ONLY_WORKLOAD) {
              /*STDIO-SHM -> STDIO-BB->STDIO-PFS*/
              TAILORFS_LOGINFO("file %s is Shared and uses STDIO",
              ...
              /*UNIFYFS-O-BB*/
              TAILORFS_LOGINFO("file %s uses UNIFYFS-O-BB",
                               file_intent._name.c_str());
              off_t mean_write_transfer_size =
                  (file_intent._write_distribution._0_4kb * 2 * KB) +
                  (file_intent._write_distribution._4_64kb * (4 + 64) / 2 *
                   KB) +
                  (file_intent._write_distribution._64kb_1mb * (64 + 1024) / 2 *
                   KB) +
                  (file_intent._write_distribution._1mb_16mb * (1 + 16) / 2 *
                   MB) +
              ...
              off_t mean_read_transfer_size =
                  ...
                  (file_intent._read_distribution._64kb_1mb * (64 + 1024) / 2 *
                   KB) +
                  (file_intent._read_distribution._1mb_16mb * (1 + 16) / 2 *
                   MB) +
              ...
              init_args.feature.enable_read_optimization =
                  mean_read_transfer_size > 0;
              auto left_shm_size =
              ...
              if (left_spill_size > 0) {
                ...
                init_args.feature.splill._mount_point =
                    spill_storage._mount_point;
              } else {
                if (storages.size() > 2) {
                  spill_storage = storages[2];
                  auto left_spill_size = spill_storage._capacity_mb -
                                         spill_storage._used_capacity_mb;
                  if (left_spill_size > 0) {
                    left_spill_size = left_spill_size > file_intent._size_mb
                                          ? file_intent._size_mb
                                          : left_spill_size;
              ...
              strcpy(
                  init_args.feature.unifyfs_namespace,
              ...
              init_args.feature.redirection.new_storage =
                  storages[storages.size() - 1];
              init_args.feature.redirection.new_storage._mount_point =
                  init_args.feature.unifyfs_namespace;
