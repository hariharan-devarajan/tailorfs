...
# MACRO tailorfs_add_c|cxx_flags
#
...
macro(tailorfs_add_cxx_flags MY_FLAGS)
  foreach(flag ${ARGN})
...
macro(tailorfs_add_c_flags MY_FLAGS)
  foreach(flag ${ARGN})
    string(FIND "${${MY_FLAGS}}" "${flag}" flag_already_set)
    if(flag_already_set EQUAL -1)
      string(REPLACE "-" "_" _CLEAN_FLAG "${flag}")
...
tailorfs_add_cxx_flags(CMAKE_CXX_FLAGS
  -fPIC -Wall -Wextra -pedantic -Wno-unused-parameter -Wnon-virtual-dtor
  -Wno-deprecated-declarations)

tailorfs_add_c_flags(CMAKE_C_FLAGS
...
if (TAILORFS_WARNINGS_AS_ERRORS)
  tailorfs_add_cxx_flags(_WERROR_FLAGS -Werror)
  separate_arguments(_WERROR_FLAGS NATIVE_COMMAND "${_WERROR_FLAGS}")
  if (NOT TARGET TAILORFS_CXX_FLAGS_werror)
    add_library(TAILORFS_CXX_FLAGS_werror INTERFACE)
    set_property(TARGET TAILORFS_CXX_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:CXX>:${_WERROR_FLAGS}>)

    add_library(TAILORFS_C_FLAGS_werror INTERFACE)
    set_property(TARGET TAILORFS_C_FLAGS_werror PROPERTY
      INTERFACE_COMPILE_OPTIONS $<$<COMPILE_LANGUAGE:C>:${_WERROR_FLAGS}>)

    # Add the "library" to the export
    install(TARGETS TAILORFS_C_FLAGS_werror EXPORT ${TAILORFS_EXPORTED_TARGETS})
    install(TARGETS TAILORFS_CXX_FLAGS_werror EXPORT ${TAILORFS_EXPORTED_TARGETS})
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
  if (USE_CLANG_LIBCXX)
    tailorfs_add_cxx_flags(CMAKE_CXX_FLAGS "--stdlib=libc++")
  else (USE_CLANG_LIBCXX)
    if (GCC_TOOLCHAIN)
      tailorfs_add_cxx_flags(CMAKE_CXX_FLAGS "--gcc-toolchain=${GCC_TOOLCHAIN}")
    endif (GCC_TOOLCHAIN)
  ...
  if (CMAKE_BUILD_TYPE MATCHES Debug)
    ...
    tailorfs_add_cxx_flags(CMAKE_CXX_FLAGS -fno-omit-frame-pointer)
    tailorfs_add_c_flags(CMAKE_C_FLAGS -fno-omit-frame-pointer)
  endif ()
endif ()
...
if (CMAKE_CXX_COMPILER_ID MATCHES "Intel")
  ...
  # -openmp_profile
  tailorfs_add_cxx_flags(CMAKE_CXX_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
  tailorfs_add_cxx_flags(CMAKE_C_FLAGS -diag-disable=2196 -wd1011 -wd1875 -diag-disable=11074 -diag-disable=11076 ${GCC_INTEROP})
endif ()
...
if (NOT TAILORFS_LIBDIR)
  if (CMAKE_INSTALL_LIBDIR)
    set(TAILORFS_LIBDIR ${CMAKE_INSTALL_LIBDIR})
  else ()
    set(TAILORFS_LIBDIR "lib")
  endif ()
endif ()

set(CMAKE_INSTALL_RPATH "${CMAKE_INSTALL_PREFIX}/${TAILORFS_LIBDIR}")

list(FIND CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES
    "${CMAKE_INSTALL_PREFIX}/${TAILORFS_LIBDIR}" _IS_SYSTEM_DIR)

if (${_IS_SYSTEM_DIR} STREQUAL "-1")
    # Set the install RPATH correctly
    list(APPEND CMAKE_INSTALL_RPATH
      "${CMAKE_INSTALL_PREFIX}/${TAILORFS_LIBDIR}")
endif ()
...
try_compile(TAILORFS_HAS_STD_FILESYSTEM "${CMAKE_BINARY_DIR}/temp"
            "${CMAKE_SOURCE_DIR}/cmake/tests/has_filesystem.cpp"
            CMAKE_FLAGS ${CMAKE_CXX_FLAGS}
            LINK_LIBRARIES stdc++fs)
if (TAILORFS_HAS_STD_FILESYSTEM)
...
endif (TAILORFS_HAS_STD_FILESYSTEM)

try_compile(TAILORFS_HAS_STD_FSTREAM_FD "${CMAKE_BINARY_DIR}/temp"
            "${CMAKE_SOURCE_DIR}/cmake/tests/has_fd.cpp"
            CMAKE_FLAGS ${CMAKE_CXX_FLAGS}
            LINK_LIBRARIES stdc++fs)
if (TAILORFS_HAS_STD_FSTREAM_FD)
...
endif (TAILORFS_HAS_STD_FSTREAM_FD)